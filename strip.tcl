namespace eval strip {
	set newcode ""
	variable slave

	# Passthrough creates a proc to pass through
	# a command to the next pass.
	proc Passthrough {name arglist} {
		set code "proc $name \{$arglist\} \{variable newcode;append newcode \"$name"
		foreach i $arglist {
			append code " \$[set i]"
		}
		append code "\\n\"\}"
		eval $code
	}

	# Bind our procs into the slave interpreter.
	proc binder {interp} {
		variable slave
		set slave $interp

		foreach i "[$slave aliases]" {
			$slave alias $i {}
		}
		foreach i {left right at @ set add subtract in out inline forceinline goto lang} {
			$slave alias $i ::strip::$i
		}
		foreach i {while set} {
			$slave alias $i ::strip::_$i
		}
	}

	# While statements, as usual, must be handled specially.
	proc _while {var code} {
		variable newcode
		variable slave

		append newcode "while $var \{\n"
		$slave eval $code
		append newcode "\}\n"
	}

	# Needs to be handled specially.
	proc _set {var num} {
		variable newcode

		append newcode "set $var $num\n"
	}

	# set up passed through commands
	foreach {name args} {goto {var num} out {var} forceinline {args} left {num} right {num} at {var} add {var num} subtract {var num} in {var} @ {var num}} {
		Passthrough $name $args
	}

	# Can't be passed through normally.
	proc lang {code} {
		variable newcode

		append newcode "lang \{$code\}\n"
	}

	# Ignored procs
	proc inline {args} {}
}
